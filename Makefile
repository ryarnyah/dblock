...
CTIMEVAR=-X $(BASE_PKG)/$(1)/pkg/version.GITCOMMIT=$(GITCOMMIT) -X $(BASE_PKG)/$(1)/pkg/version.VERSION=$(VERSION)
GO_LDFLAGS=-ldflags "-w $(call CTIMEVAR,$(1))"
GO_LDFLAGS_STATIC=-ldflags "-w $(call CTIMEVAR,$(1)) -extldflags -static"

# List the GOOS and GOARCH to build
GOOSARCHES = linux/amd64 linux/386 linux/arm linux/arm64 windows/amd64 windows/386 darwin/amd64
